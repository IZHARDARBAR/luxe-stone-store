...
import { ArrowRight, Star, Truck, ShieldCheck, Box, CreditCard } from 'lucide-react';
...
const Home = () => {
  ...
  const saleImage = saleProduct?.images && saleProduct.images.length > 0 
      ...
      <section className="relative h-screen w-full flex items-center justify-center text-center">
        ...
        <div className="relative z-10 text-white px-4 max-w-4xl mx-auto mt-16">
          ...
          <p className="text-lg text-gray-200 mb-8 max-w-2xl mx-auto">
            From Electronics to Fashion, discover premium quality products at the best prices in Pakistan.
          </p>
          <Link to="/shop">
      ...
      */}
      <section className="py-16 bg-white border-b border-gray-100">
        <div className="container mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          <FeatureItem icon={<CreditCard />} title="Secure Payment" sub="COD & Bank Transfer" />
          <FeatureItem icon={<Truck />} title="Fast Delivery" sub="All over Pakistan" />
          <FeatureItem icon={<Box />} title="Easy Returns" sub="7 Days Checking" />
          <FeatureItem icon={<ShieldCheck />} title="Verified Products" sub="100% Original" />
        </div>
      </section>
      ...
      <section className="py-20 container mx-auto px-6">
        ...
        {trendingProducts.length > 0 ? (
          ...
          <p className="text-center text-gray-500">Check back later for trending items.</p>
        )}
      </section>
      ...
      {saleProduct && (
        <section className="relative py-28 bg-gray-900 text-white text-center">
          ...
          <div className="relative z-10 px-6">
            <span className="bg-red-600 text-white px-4 py-1 uppercase text-xs font-bold tracking-widest mb-4 inline-block">Flash Sale</span>
            <h2 className="text-4xl md:text-6xl font-bold mb-4">{saleProduct.name}</h2>
            <p className="text-xl text-gray-300 mb-8 max-w-2xl mx-auto">{saleProduct.description ? saleProduct.description.slice(0, 100) : ''}...</p>
...
const FeatureItem = ({ icon, title, sub }) => (
  <div className="flex flex-col items-center text-center p-4">
    <div className="mb-3 text-black bg-[#f4f7eb] p-3 rounded-full">{React.cloneElement(icon, { size: 28 })}</div>
  ...
  return (
    <Link to={`/product/${product.id}`} className="group relative block">
      ...
      <div className="mt-5 text-center">
        ...
        <p className="mt-2 text-lg font-semibold text-[#84a93e]">Rs. {product.price}</p>
      </div>
    </Link>
  );
};
