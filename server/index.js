...
// 1. Test Route
app.get('/', (req, res) => {
  res.json({ message: "Luxe Stone Email Server is Running!" });
});
...
app.post('/api/contact', async (req, res) => {
  ...
  // Check agar password missing hai
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    return res.status(500).json({ success: false, message: "Server Config Error: Missing Email/Pass" });
  }
